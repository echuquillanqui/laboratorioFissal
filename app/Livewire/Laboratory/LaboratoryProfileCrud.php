...
public function render(){return view('livewire.laboratory.profile-crud',['profiles'=>LaboratoryProfile::withTrashed()->with('tests:id,nombre')->when($this->search!=='',fn($q)=>$q->where('nombre','like',"%{$this->search}%"))->orderByDesc('id')->paginate(10),'tests'=>LaboratoryTest::where('estado',true)->orderBy('nombre')->get(['id','nombre'])])->layout('layouts.app');}}
