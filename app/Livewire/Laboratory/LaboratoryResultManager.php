...
public function render(){return view('livewire.laboratory.result-manager',['orders'=>LaboratoryOrder::with('patient:id,nombres_apellidos')->latest()->get(['id','patient_id','fecha_orden']),'order'=>$this->orderId?LaboratoryOrder::with('items.test.options','items.result')->find($this->orderId):null])->layout('layouts.app');}
}
